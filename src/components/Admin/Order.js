...
import Detail from "./Detail_order";
...
class Order extends React.Component {
    ...
    componentDidMount() {
        ...
        getCollection('don-hang').then(res => {
            this.setState({
                data: res,
            })
        })
    ...
    Dangki = (event) => {
        ...
        createAccount(this.state.input_email, this.state.input_password2).then(uid => {
            let data = {
                id: uid,
                                ...
                                <div className="card-header">
                                    <div className="d-flex align-items-center justify-content-between">
                                        ...
                                        <div className='d-flex'>
                                            <form className="d-none d-flex mx-5">
                                            ...
                                            <button type="button" className="d-none btn btn-dark" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                                Thêm tài khoản
                                            </button>
                                            <button className="d-none btn btn-dark mx-2" onClick={this.selected} disabled={this.state.view_detail ? true : false} type="button">{this.state.select ? "Bỏ chọn" : "Chọn"}</button>
                                            <button className="d-none btn btn-dark" onClick={this.Delete} disabled={this.state.isCheck.length != 0 ? false : true} type="submit">Xoá</button>
                                        </div>
                                    </div>
                        ...
                        <div className="modal-dialog modal-lg">
                            <div className="modal-content">
                                <div className="modal-header">
                                    <h5 className="modal-title" id="exampleModalLabel">Đăng kí tài khoản</h5>
                                ...
                                <div className="modal-body">
                                    <form className="row" onSubmit={this.Dangki}>
                                        <div className="col-6">
                                            <div className="mb-3">
                                                <label htmlFor="displayName" className="form-label">Tên người dùng</label>
                                                <input required type="text" className="form-control" name="input_name" onChange={(event) => this.change_input(event)} />
                                            </div>
                                            <div className="mb-3">
                                                <label htmlFor="image" className="form-label">Hình đại diện</label>
                                                <input type="text" className="form-control" onChange={(event) => this.change_input(event)} name="input_image" />
                                            </div>
                                            <div className="mb-3">
                                                <label htmlFor="phone" className="form-label">Số điện thoại</label>
                                                <input required type="number" className="form-control" onChange={(event) => this.change_input(event)} name="input_phone" />
                                            </div>
                                        </div>
                                        <div className="col-6">
                                            <div className="mb-3">
                                                <label htmlFor="hidden" className="form-label">Email</label>
                                                <input required type="email" className="form-control" onChange={(event) => this.change_input(event)} name="input_email" />

                                            </div>
                                            <div className="mb-3">
                                                <label htmlFor="password" className="form-label">Mật khẩu</label>
                                                <input required type="password" className="form-control" onChange={(event) => this.change_input(event)} name="input_password" />
                                            </div>
                                            <div className="mb-3">
                                                <label htmlFor="password2" className="form-label">Xác nhận mật khẩu ({this.state.input_password == this.state.input_password2 ? <span className="text-success">Khớp</span> : <span className="text-danger">Chưa khớp</span>})</label>
                                                <input required type="password" className="form-control" onChange={(event) => this.change_input(event)} name="input_password2" />
                                            </div>
                                        </div>
                                        <div className="d-flex justify-content-end">
                                            <button type="submit" className="mx-1 btn btn-primary">Đăng kí</button>
